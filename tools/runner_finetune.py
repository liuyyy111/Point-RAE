...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            # points = data[0].cuda()
            # label = data[1].cuda()
            new_data = mixup.rsmix(data)
            points = new_data['pc'].cuda()
            ...
            loss, acc = base_model.module.get_loss_acc(ret, new_data)
...
#                 train_writer.add_scalar('Loss/Batch/TrainAcc', acc, n_itr)
        ...
        print_log('[Training] EPOCH: %d ETA : %s EpochTime : %.3f (s) [Loss] = %s lr = %e' %
        ...
        if epoch % args.val_freq == 0 and epoch != 0:
            ...
            if args.vote:
                if metrics.acc > 89.1 or (better and metrics.acc > 89.1):
