...
Collected and written by: Charles R. Qi and Or Litany
"""
...
# random sampling
def random_sampling(pc, num_sample, feat=None, replace=None, return_choices=False):
    ...
    if feat is not None:
        if return_choices:
            return pc[choices], feat[choices], choices
        else:
            return pc[choices], feat[choices]
    else:
        if return_choices:
...
# FPS sampling
def farthest_point_sample(point, npoint, return_choices=False):
    """
    Input:
