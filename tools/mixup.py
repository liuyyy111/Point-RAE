...
def rsmix(data, n_sample=512, KNN=False):
    cut_rad = np.random.beta(1., 1.)
    data_batch = data[0].numpy()
    label_batch = data[1].numpy()
    ...
    new_data = {}
    new_data['pc'] = torch.FloatTensor(data_batch_mixed)
    new_data['label'] = torch.tensor(label_a)
    new_data['label_2'] = torch.tensor(label_b)
    new_data['lam'] = torch.tensor(lam)
