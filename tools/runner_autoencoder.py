...
from models.dvae import chamfer_distance_l2, chamfer_distance_l1
import math
import cv2
import numpy as np
...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    # Criterion
    ChamferDisL1 = chamfer_distance_l1
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        time_sec_avg = time_sec_tot / (
            epoch - start_epoch + 1)
        eta_sec = time_sec_avg * (config.max_epoch - epoch - 1)
        eta_str = str(datetime.timedelta(seconds=int(eta_sec)))

        print_log('[Training] EPOCH: %d ETA : %s EpochTime : %.3f (s) Losses = %s lr = %.6f' %
...
def validate(base_model, test_dataloader, epoch, ChamferDisL1, ChamferDisL2, val_writer, args, config, logger = None):
    ...
    for taxonomy_id in category_metrics:
        taxonomy_table = []
        taxonomy_table.append(taxonomy_id)
        taxonomy_table.append(str(category_metrics[taxonomy_id].count(0)))
        for value in category_metrics[taxonomy_id].avg():
            taxonomy_table.append('%.3f' % value)
        taxonomy_table.append(shapenet_dict[taxonomy_id])
        table_data.append(taxonomy_table)
    
    overall_result = ['Overall', '--']
    for value in test_metrics.avg():
        overall_result.append('%.3f' % value)
    overall_result.append('--')
    table_data.append(overall_result)
...
def validate_net(args, config):
    ...
    validate(base_model, test_dataloader, 0, ChamferDisL1, ChamferDisL2, None, args, config, logger=logger)
